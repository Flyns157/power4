...
def init(h: int = 6, w: int = 7, debug: bool = False) -> list:
    if debug:
        return [[i for i in range(h)] for _ in range(w)]
    else:
        return [list([B]*h) for _ in range(w)]


def visual(L: list[list[str]])-> None:
    nav.write("<main>")
    ...
    if enter != 9 and B in L[enter]:
        ...
        if estRempli(L) or rangee4(L, j) or colonne4(L, j) or diagonal4(L, j):
            ...
            if rangee4(L, j) or colonne4(L, j) or diagonal4(L, j):
                nav.write(
                    """<h1 style="color: red;">They won !!!</h1>""")
            elif estRempli(L):
...
def lastinc(L: list[list[str]], c: int)-> int:
    i = len(L[0])-1
...
def update(L: list[list[str]], j: int, c: int)-> None:
    L[c][lastinc(L, c)] = ["rouge", "jaune"][j]


def estRempli(L: list[list[str]]) -> bool:
    return B not in [L[i][-1] for i in range(len(L))]
...
def rangee4(L: list[list[str]], j: int) -> bool:
    for k in range(len(L[0])):
        ...
        for i in range(len(L)):
            n = n+1 if L[i][len(L[0])-1-k] == ["rouge",
                                               "jaune", "fond"][j] else 0
            if n == 4:
                return True
    return False
...
def diagonal4(L: list[list[str]], j: int) -> bool:
    ...
    for x in range(1+len(L)-4):
        y, n = 0, 0
        while x+y < len(L) and y < len(L[0]):
            n = n+1 if L[len(L)-1-x-y][y] == ["rouge",
                                              "jaune", "fond"][j] else 0
            if n == 4:
                return True
            y += 1
    for y in range(1, 1+len(L[0])-4):
        x, n = 0, 0
        while x < len(L) and y+x < len(L[0]):
            n = n+1 if L[len(L)-1-x][x+y] == ["rouge",
                                              "jaune", "fond"][j] else 0
            if n == 4:
                return True
            x += 1
...
def navsend(P: list[list[str]], j: int, typesend: str)-> None:
    if typesend == 'form':
        nav.write(
            f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Connect 4</title><link rel="stylesheet" href="style.css"></head><body><h1>Round number {n}, it's {["reds","yellows"][j]}' turn to play :</h1>""")
        visual(P)
        nav.write("""<form method="POST" id="form1"><input type="submit" name="colone" value="1"><input type="submit" name="colone" value="2"><input type="submit" name="colone" value="3"><input type="submit" name="colone" value="4"><input type="submit" name="colone" value="5"><input type="submit" name="colone" value="6"><input type="submit" name="colone" value="7"></form></body></html>""")
        nav.endPage()
    elif typesend == 'link':
        nav.write(
            f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Connect 4</title><link rel="stylesheet" href="style.css"></head><body><h1>Round number {n}, it's {["reds","yellows"][j]}' turn to play :</h1><section><a href="/0"><div id="{["red","yellow"][j]}choice"></div></a><a href="/1"><div id="{["red","yellow"][j]}choice"></div></a><a href="/2"><div id="{["red","yellow"][j]}choice"></div></a><a href="/3"><div id="{["red","yellow"][j]}choice"></div></a><a href="/4"><div id="{["red","yellow"][j]}choice"></div></a><a href="/5"><div id="{["red","yellow"][j]}choice"></div></a><a href="/6"><div id="{["red","yellow"][j]}choice"></div></a></section>""")
        visual(P)
        nav.write("""<</body></html>""")
        nav.endPage()
    else:
